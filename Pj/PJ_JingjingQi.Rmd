...
```{r}
corr_coeff <- cor(assay(DESeq.poly_a, "log.norm.counts"), method = "pearson")
...
poly_a.results <- results(DESeq.poly_a, independentFiltering = TRUE, alpha = 0.05)
summary(poly_a.results)
table(poly_a.results$padj < 0.05)
hist(poly_a.results$padj)
plotMA(poly_a.results, alpha = 0.05,
